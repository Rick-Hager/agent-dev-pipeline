...
Pix é um meio de pagamento eletrônico instantâneo oferecido pelo Banco Central do Brasil a pessoas físicas e jurídicas. Através do Checkout Transparente, é possível oferecer esta opção de pagamento por meio de QR Code ou código de pagamento.
...
Para integrar pagamentos via Pix, siga as etapas abaixo, mas caso você já tenha integrado pagamentos via cartão, inicie a integração a partir da etapa [Adicionar formulário de pagamento](/developers/pt/docs/checkout-api-payments/integration-configuration/integrate-with-pix#bookmark_Adicionar_formulário_de_pagamento).
...
Para configurar pagamento com Pix, envie um POST ao endpoint [/v1/payments](/developers/pt/reference/online-payments/checkout-api-payments/create-payment/post) e execute a requisição ou, se preferir, faça a requisição utilizando nossos SDKs.
...
A resposta mostrará o estado pendente do pagamento e todas as informações que você precisa para mostrar ao comprador. O valor `transaction_data` retornará os dados para QR Code.
...
Para o usuário efetuar o pagamento, você deve escolher a forma de abertura do mesmo, que pode ser através de um botão ou de um QR Code que deve ser renderizado. 
...
* **Renderizar QR Code**: É possível renderizar o QR Code vigente, que deverá ser utilizado somente uma vez, na própria tela. Além disso, também é possível adicionar uma opção para copiar e colar o código de pagamento, o que permitirá realizar a transação a partir de um Internet Banking.

Siga as etapas abaixo para renderizar o QR code e disponibilizar o recurso copia e cola.

1. Adicione o `qr_code_base64` para exibir o QR Code.
...
Ao concluir essas etapas, o QR Code terá sido renderizado e será exibido para o comprador no momento do pagamento.